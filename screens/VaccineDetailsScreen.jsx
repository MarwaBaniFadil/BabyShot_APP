...
import ipAdd from 'C:/Users/ELIFE STORE/children_vaccination_clinic/scripts/helpers/ipAddress';
 ...
 const App = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchParentIdAndChildren = async () => {
      try {
        ...
        console.log("البيانات المستلمة من الـ API:", response.data.users);
  ...
  const handleChildSelect = async (child) => {
    try {
      if (!child.child_id) {
        console.error("ID الطفل غير موجود");
        return;
      }

      const childId = child.child_id;
      console.log("تم اختيار الطفل، ID:", childId);

      const vaccinesResponse = await axios.get(
        `${ipAdd}:8888/APIS/vaccineschild/${childId}`
      );

      if (vaccinesResponse.status === 200 && vaccinesResponse.data.users) {
        console.log("نتيجة الـ API (اللقاحات):", vaccinesResponse.data.users[0]);
        setChildVaccines(vaccinesResponse.data.users[0]);
        setShowOverlay(true); // عرض نافذة التطعيمات
        setShowNextAgeOverlay(false); // إغلاق نافذة العمر التالي
      } else {
        console.error("الاستجابة غير متوقعة أو لا تحتوي على بيانات صحيحة.");
      }

      const nextAgeResponse = await axios.get(
        `${ipAdd}:8888/APIS/getNextAge/${childId}`
      );
      
      let nextAge = null;
      
      if (nextAgeResponse.status === 200) {
        if (nextAgeResponse.data.message === "جميع الأعمار تم تسجيلها بالفعل.") {
          nextAge = "لا يوجد";
          console.log("الطعم القادم:", nextAge);
        } else if (nextAgeResponse.data.nextAge) {
          nextAge = ageMapping[nextAgeResponse.data.nextAge] || "غير معروف";
          console.log("العمر التالي:", nextAge);
        } else {
          console.error("الاستجابة للطعم القادم غير متوقعة:", nextAgeResponse.data);
        }
      } else {
        console.error("حالة الاستجابة غير متوقعة:", nextAgeResponse.status);
      }
      ...
      // التحقق من حالة "لا يوجد" للطعم القادم
      if (nextAge === "لا يوجد") {
        vaccineDate = "لا يوجد";
        console.log("تاريخ اللقاح القادم:", vaccineDate);
      } else if (
        dateResponse.status === 200 &&
        dateResponse.data.nextVaccineDate
      ) {
        vaccineDate = dateResponse.data.nextVaccineDate;
        console.log("تاريخ اللقاح القادم:", vaccineDate);
      } else {
        console.error("الاستجابة للتاريخ غير متوقعة.");
      }

      setSelectedChild(child.name);
      setNextAge(nextAge);
      setNextVaccineDate(vaccineDate);
      setShowNextAgeOverlay(true); // عرض نافذة العمر والتاريخ
      setShowOverlay(false); // إغلاق نافذة التطعيمات
    } catch (error) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollViewContainer}>
      ...
      <View style={styles.childrenContainer}>
        <Text style={styles.boldText}>اختر الطفل من هنا لترى الطعومات التي تلقاها  </Text>
        {children.map((child, index) => (
          ...
          >
            <Text style={styles.childButtonText}>{child.name}</Text>
          </TouchableOpacity>
        ))}
      </View>
...
{showNextAgeOverlay && (
  ...
  onPress={() => navigation.navigate('Sch')} // تأكد أن 'Sch' هو اسم الشاشة في الستاك
>
  <Text style={styles.bookAppointmentText}>احجز موعد من هنا</Text>
</TouchableOpacity>
        </>
      ) : (
